...
image::19-network-policies.png[NetworkPolicy micro-segmentation,960]

.Policy demo in the lab
[cols="1,1", options="header"]
|===
| Rule | Manifest

| Target workload
| `line-dashboard` pods in `industrial-edge-tst-all`

| Allowed ingress
| OpenShift ingress namespace + `machine-sensor` pods only
...
This workshop applies a demo NetworkPolicy in `industrial-edge-tst-all` from `charts/all/workshop-demos/templates/network-policy-demo.yaml`, allowing dashboard ingress while denying unexpected cross-namespace traffic. As `%USER_NAME%`, test allowed and denied paths using `oc exec` curl probes from the Showroom terminal.
...
. Governance protects `%USER_NAME%`'s namespace — NetworkPolicy demo in `industrial-edge-tst-all`.
. Apply or review demo NetworkPolicy; run allowed vs denied curl from Showroom terminal.
. Relate to ROSA security groups + NP defense in depth.
. Preview ACS (module 20) and Kuadrant API keys as additional layers.
...
| NP demo | `charts/all/workshop-demos/templates/network-policy-demo.yaml` |
