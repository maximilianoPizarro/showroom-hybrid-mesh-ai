...
image::18-scalability.png[HPA and Kafka scaling,960]
...
. Under load, `%USER_NAME%`'s line-dashboard scales horizontally — HPA after Kairos rightsizing (module 14).
. Watch HPA scale line-dashboard pods under simulated sensor load.
. Show Kafka consumer lag recovering after buffer capacity absorbs burst.
. Tie pod-scale (HPA) vs node-scale (Kairos) layering for factory edge.
...
| IE workloads | `charts/all/industrial-edge-tst/` |
