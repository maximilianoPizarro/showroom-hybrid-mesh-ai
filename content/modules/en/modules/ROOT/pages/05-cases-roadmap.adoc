...
Your next step is Part B registration verification — ensure `pass:[%USER_NAME%]` works in Showroom, then proceed to module 10 for ACM fleet visibility. Plan B shared demos remain available if your scaffold slot is unavailable.
...
| Registration (start Part B) | `https://workshop-registration.{hub_domain}/` | Email → pass:[%USER_NAME%]
| Showroom | `https://showroom-showroom.{hub_domain}/?USER_NAME=pass:[%USER_NAME%]` | After redirect
...
. Transition room to Part B: verify `pass:[%USER_NAME%]` login before module 10.

== Your TODO (user pass:[%USER_NAME%])

* [ ] Write one metric from the manufacturing case relevant to your industry
* [ ] Confirm Showroom login as `pass:[%USER_NAME%]` before module 10
