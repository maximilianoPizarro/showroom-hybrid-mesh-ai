...
image::11-arch-spoke-gateway.png[Hub gateway to spoke Industrial Edge,640]

image::11-arch-skupper-topology.png[Skupper VAN — three sites, kafka and IE listeners,640]
...
| External client → hub
| OpenShift Route + `hub-gateway` Gateway (`charts/all/hub-gateway/`)

| Hub ingress routing
| HTTPRoute in `hub-gateway-system`

| Cross-cluster link
| Skupper VAN / Connectivity Link (`charts/all/service-interconnect/`)
...
[source,yaml]
----
# charts/all/hub-gateway/templates/gateway.yaml
...
[source,yaml]
----
# charts/all/workshop-kuadrant-apis/templates/routes.yaml (pattern)
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
  name: workshop-{{ $name }}
...
*Skupper Sites and Connectors* in `charts/all/service-interconnect/` bridge the hub-gateway-system to spoke namespaces, carrying traffic for IE frontends, Kafka Console, and cross-cluster observability — all encrypted over the Skupper tunnel without VPC peering.
...
| Skupper network observer | link:https://skupper-network-observer-service-interconnect.%HUB_DOMAIN%[Skupper network observer] | OpenShift OAuth
...
. `%USER_NAME%`'s factory apps will reach the hub through one Gateway — no VPC peering between plants and cloud.
. Trace external URL → hub HTTPRoute → Skupper VAN → spoke IE frontend.
. Display Skupper `sitesInNetwork: 3` in console observer.
. Show Validated Patterns region layout: `charts/region/hub`, `charts/region/east`, `charts/region/west`.
. Relate to ROSA ALB + private link narrative from Part A module 02.
...
== Validated Patterns region layout

The mesh architecture maps to the VP single-branch region strategy:
...
[source,bash]
----
# Region bootstrap paths (single branch: main)
charts/region/hub/    # ACM, Developer Hub, ACS Central, hub-gateway, OpenShift AI
charts/region/east/   # Industrial Edge, Skupper connector, DevSpaces
charts/region/west/   # IE replica, Skupper connector
charts/all/           # 50+ shared Helm charts synced by Argo CD
----

See link:https://validatedpatterns.io[Validated Patterns] and link:https://github.com/maximilianoPizarro/hybrid-mesh-platform/blob/main/docs/validatedpatterns-docs/architecture.md[Architecture docs].
...
| Hub gateway | `charts/all/hub-gateway/` |
| Skupper | `charts/all/service-interconnect/` |
|===
