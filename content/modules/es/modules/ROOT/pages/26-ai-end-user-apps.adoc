= IA en apps finales

span.workshop-time-badge[~20 min]

NOTE: Para la tabla ROSA/AWS vs lab ver xref:00-index.adoc#hybrid-callout[Módulo 00 — tabla comparativa].

== Panorama

Las aplicaciones finales — dashboards de operador, alertas móviles, integraciones MES — consumen insights de IA donde ocurre el trabajo, no solo en notebooks de data science. line-dashboard en spoke east visualiza streams Kafka de sensores; integraciones Camel enrutan eventos de anomalía; NeuroFace y resúmenes MaaS se embeben en la misma UX que `pass:[%USER_NAME%]` ve en rollouts de producción.

Este módulo integra módulos 13–25: verifica que line-dashboard muestra datos IE en vivo, dispara umbrales ie-anomaly-alerter y opcionalmente embebe iframe NeuroFace o enlace chat para ayuda IA contextual. Integraciones Camel K de plantillas `demo-camel-kaoto-east` y `demo-camel-cdc-east` muestran patrones event-driven que equipos de fábrica despliegan en OpenShift junto a sistemas OT.

Los equipos de plataforma ganan cuando la IA es infraestructura invisible: OpenShift AI, MaaS y NeuroFace son entradas de catálogo — squads de app los vinculan vía dependencias Developer Hub sin reconstruir pipelines ML por planta.

== Acceso al producto — URLs y credenciales

NOTE: *Login del taller* — pass:[%USER_NAME%] / `Welcome123!` (Keycloak en Developer Hub; htpasswd en consola OpenShift hub/east/west; DevSpaces en spoke east). Productos OAuth usan la misma identidad OpenShift.

[cols="2,3,2"]
|===
| Servicio | URL | Credenciales

| Dashboard línea IE | `https://industrial-edge.{hub_domain}` | Pública (sin login)
| Developer Hub — componentes IE | `https://developer-hub.{hub_domain}/catalog` | Usuario taller
| Grafana (telemetría) | `https://grafana.{hub_domain}` | OAuth OpenShift
|===

== Show and Tell

. Datos en vivo line-dashboard + overlay anomalía o badge alerta.
. Opcional estado integración Camel en Topology.
. Historia: operador ve telemetría, predicción y ayuda IA en una UX.

== Tu TODO (usuario pass:[%USER_NAME%])

* [ ] Verificar que line-dashboard muestra telemetría IE en vivo
* [ ] Conectar un insight de anomalía o IA al flujo del operador
* [ ] Guardar progreso al final de este módulo

== Lab en vivo

* Terminal `oc` integrada en Showroom — usa contexto hub salvo que el módulo indique east/west.

== YAML behind the scenes

[cols="2,3,1"]
|===
| UI action | Git source | Kind

| line-dashboard | industrial-edge-tst-all | Deployment |
| Camel K | industrial-edge-pipelines | Integration |
|===

[source,yaml]
----
# End-user apps on spoke consume IE + AI stack
----

Verify:

[source,bash]
----
oc get deploy -n industrial-edge-tst-all line-dashboard
----

== Verificar

[cols="1,2,1"]
|===
| Check | Action | Expected

| Progress | Save checkboxes below | `POST /api/progress` returns OK
|===


++++
<div class="workshop-progress" data-module="26-ai-end-user-apps">
  <label><input type="checkbox" data-completed> Completé este módulo</label>
  <label><input type="checkbox" data-interest> Me interesa profundizar</label>
  <button type="button" onclick="saveWorkshopProgress('26-ai-end-user-apps')">Guardar progreso</button>
</div>
++++


TIP: Plan B — demo compartido en Developer Hub → System `hybrid-mesh-shared-demos` si falla el scaffolder.


++++
<div class="workshop-next-nav">
  <a href="27-full-verification.html">Siguiente → 27 full verification</a>
</div>
++++

