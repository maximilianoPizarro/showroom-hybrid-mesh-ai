...
In this lab, open ACM Clusters on the hub and locate east and west — each spoke was bootstrapped from `components/acm-hub-spoke/` GitOps manifests. As `pass:[%USER_NAME%]`, your workloads land on east by default; Topology in Developer Hub uses OCM APIs to show the same graph ACM displays.
