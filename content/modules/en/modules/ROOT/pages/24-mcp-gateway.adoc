...
image::24-mcp-gateway.png[MCP Gateway — Argo CD and k8s tools for Lightspeed,960]

.MCP tools in the lab
[cols="1,1", options="header"]
|===
| Tool server | Purpose

| Argo CD MCP
| Query/sync Applications from Lightspeed prompts

| k8s MCP
| Cluster resources without raw `oc` in chat

| OpenShift AI MCP
| Register in dashboard — connects to MaaS

| Public endpoint
| `https://mcp-gateway.%HUB_DOMAIN%/mcp`
|===
...
**Hands-on (~30 min):** Developer Hub **/lightspeed** — activity: "List Argo CD apps in OutOfSync state and suggest sync." Llama-stack uses `remote::mcp` to gateway (`charts/all/developer-hub/files/lightspeed/llama-stack-run.yaml`).

The configuration is declarative and minimal:

[source,yaml]
----
# charts/all/mcp-gateway/templates/mcp-server-registration.yaml
...
*MCPGatewayExtension* — binds the Gateway API ingress to the MCP controller:

[source,yaml]
----
# charts/all/mcp-gateway/templates/mcp-gateway-extension.yaml
apiVersion: kuadrant.io/v1alpha1
...
spec:
  gatewayRef:
    name: mcp-gateway              # Gateway API Gateway in mcp-system
    namespace: mcp-system
  listenerPort: 8080                 # MCP JSON-RPC over HTTP
  auth:
    enabled: true                    # workshop users authenticate via Developer Hub OIDC
----

*Llama Stack tool binding* — Lightspeed routes prompts to MCP tools via `remote::mcp`:

[source,yaml]
----
# charts/all/developer-hub/files/lightspeed/llama-stack-run.yaml (excerpt)
tool_groups:
  - toolgroup_id: remote::mcp
    provider_id: mcp
    args:
      url: https://mcp-gateway.%HUB_DOMAIN%/mcp
      transport: streamable-http
model: granite-3.1-8b-instruct       # aligned with Kairos / Lightspeed sidecar
inference_store:
  type: sqlite
  db_path: /tmp/lightspeed.db
----

=== Learn more

* link:https://modelcontextprotocol.io/[Model Context Protocol (MCP) specification]
* link:https://modelcontextprotocol.io/specification/2025-03-26/server/tools[MCP tools specification]
...
. Frame MCP as **safe automation for `%USER_NAME%`** — natural language triggers scoped tools, not raw cluster-admin `oc`.
. `oc get mcpgatewayextension,mcpserverregistration -n mcp-system`.
. Developer Hub `/lightspeed` — prompt: "List Argo CD apps in OutOfSync state and suggest sync."
. Show `llama-stack-run.yaml` tool_groups binding to `mcp-gateway.%HUB_DOMAIN%`.
. OpenShift AI dashboard → register `ods-maas-mcp-server` MCP URL from maas-workshop ConfigMap.
...
| MCP CRDs + controller | `charts/all/mcp-gateway/` |
