...
| RHCL Kuadrant — modules 20/23, keys via Developer Hub

| Published factory APIs
| APIProduct `demo-ie-api-product` via Connectivity Link on hub gateway
|===
...
. Security stack for `%USER_NAME%`: ACS scans runtime threats; Kuadrant governs API access including future AI routes.
. ACS Central overview — violations and deployments on east/west spokes.
. Demo APIProduct route via hub gateway with AuthPolicy (preview module 23 AI Gateway).
. Remind ACS `stackrox` namespace stays outside ambient mesh.
...
| Workshop APIs | `charts/all/workshop-kuadrant-apis/` |
